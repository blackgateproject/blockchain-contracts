...
const Blackgate = buildModule("Blackgate", (m) => {
  const modulus =
  ...
  // Deploy RSAAccumulator
  const RSAAccumulatorVerifier = m.contract("RSAAccumulatorVerifier", [
    modulus,
  ]);
